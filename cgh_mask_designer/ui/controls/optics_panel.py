...
class OpticsPanel(QtWidgets.QWidget):
    def __init__(self, st, on_change):
        ...
        self.model_cb = QtWidgets.QComboBox(); self.model_cb.addItems(["Fourier","Fresnel"]); self.model_cb.setCurrentText(st.model)
        ...
        self.recon_res_cb = QtWidgets.QComboBox(); self.recon_res_cb.addItems(["Full pixel", "Sub-pixel"]); self.recon_res_cb.setCurrentText(st.recon_resolution); self.recon_res_cb.currentIndexChanged.connect(self.on_change)
        ...
        form.addRow("Reconstruction resolution", self.recon_res_cb)
